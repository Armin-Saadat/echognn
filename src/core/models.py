...
class MLPEdgeEncoder(nn.Module):
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        x_prime1 = torch.zeros(x.size(0), self.num_frames//2, self.hidden_dim).to(self.device)
        x_prime2 = torch.zeros(x.size(0), self.num_frames//2, self.hidden_dim).to(self.device)
        for i in range(self.num_frames//2):
            x1 = x.view(x.size(0), self.num_frames, self.num_frames-1, self.hidden_dim)[:, :self.num_frames//2, -self.num_frames//2:, :].permute(0, 2, 1, 3)
            x2 = x.view(x.size(0), self.num_frames, self.num_frames-1, self.hidden_dim)[:, self.num_frames//2:, :self.num_frames//2, :].permute(0, 2, 1, 3)
            if i==0:
                ...
                x_prime1 += torch.concat((torch.diagonal(x1, offset=i, dim1=-3, dim2=-2).permute(0, 2, 1), torch.diagonal(x1, offset=i-self.num_frames//2, dim1=-3, dim2=-2).permute(0, 2, 1)), dim=1)
                x_prime2 += torch.concat((torch.diagonal(x2, offset=i, dim1=-3, dim2=-2).permute(0, 2, 1), torch.diagonal(x2, offset=i-self.num_frames//2, dim1=-3, dim2=-2).permute(0, 2, 1)), dim=1)
        ...
        x_fin[:, :self.num_frames//2, :-self.num_frames//2, :] = x.view(x.size(0), self.num_frames, self.num_frames-1, self.hidden_dim)[:, :self.num_frames//2, :-self.num_frames//2, :]
        x_fin[:, self.num_frames//2:, self.num_frames//2:, :] = x.view(x.size(0), self.num_frames, self.num_frames-1, self.hidden_dim)[:, self.num_frames//2:, self.num_frames//2:, :]
        x_fin[:, :self.num_frames//2, -self.num_frames//2:, :] = x_fin1
        x_fin[:, self.num_frames//2:, :self.num_frames//2, :] = x_fin2
        print(x_fin[0, :4, -self.num_frames//2:-self.num_frames//2+3, 1])
        print(x_fin[0, self.num_frames//2:self.num_frames//2+3, :3, 3])
